...
- 非掲載25件の復帰: ユーザー実走GPX(井の頭・彩湖方式が最良) or Stravaセグメント再探索(APIトークン再発行が必要) or gpx.studio手動トレースでコースを整備できたものから `is_published = true` に戻す。複数周回ログの1周切り出しは実績あり(6周→中央値の周を採用)
