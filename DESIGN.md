...
旅先・出張先・大会遠征など、知らない土地でも走る場所を決めて、そのまま走り始められるWebサービス。
...
- **一言で**: 知らない土地でも、走る場所を決めて、そのまま走り始められるランニングコース検索サービス。
- **提供価値**: エリア・大会・条件から候補を絞り、代表コースとスタート地点を確認して、現地まで迷わず移動できる。コミュニティ機能は走る判断を支える副次的な価値として配置する。
...
1. **ヒーローセクション**: 背景 `brand`(黄)。中央にキャッチコピー
   - 大見出し: 「次はどこでランする？」
   - 小見出し: 「旅先でも、出張先でも、大会遠征でも。知らない土地で走れるコースが地図つきで見つかる」
...
4. **大会からさがす**: 大会遠征の試走・前日ランに使えるコースを大会別に並べる。
5. **エリアからさがす**: 地方ブロック(北海道・東北 / 関東 / 中部 / 近畿 / 中国・四国 / 九州・沖縄)ごとに、**スポットが1件以上ある都道府県のみ**「東京都 (12)」形式で件数付きリンクを並べる。
6. **条件からさがす**: 距離・設備・路面など、走る判断に直結する条件を並べる。
7. **人気スポット**: 人気順のスポットカードを表示し、人気順を保った一覧へリンクする。
8. **新着スポット**: 最新スポットをカードで表示する。
