...
export function SearchFilters({ tags, params }: { tags: Tag[]; params: Params }) {
  const [selectedTags, setSelectedTags] = useState(() => (params.tags ?? "").split(",").filter(Boolean));
  ...
  const toggleTag = (slug: string) => setSelectedTags((current) => (current.includes(slug) ? current.filter((value) => value !== slug) : [...current, slug]));
  const trackSearch = (event: React.FormEvent<HTMLFormElement>) => {
    try {
      const data = new FormData(event.currentTarget);
      const meta = { q: (data.get("q") as string) || undefined, pref: (data.get("pref") as string) || undefined, tags: selectedTags.join(",") || undefined };
      navigator.sendBeacon?.("/api/track", new Blob([JSON.stringify({ name: "search", path: "/spots", meta })], { type: "application/json" }));
    } catch {}
  };
  return (
    <form action="/spots" onSubmit={trackSearch} className="space-y-5 rounded-xl border border-line bg-paper p-4 sm:p-5">
      ...
      <details className="group" open={Boolean(params.tags || params.type || params.dist || params.toilet || params.locker || params.sento)}>
        ...
        <div className="mt-4 space-y-5 border-t border-line pt-4">
          <fieldset><legend className="mb-2 text-sm font-bold">タグ（複数選択はAND検索）</legend><div className="flex flex-wrap gap-2">{tags.map((tag) => <label key={tag.id} className="cursor-pointer"><input type="checkbox" checked={selectedTags.includes(tag.slug)} onChange={() => toggleTag(tag.slug)} className="peer sr-only" /><span className="inline-flex rounded-full bg-cream px-3 py-1.5 text-sm peer-checked:bg-brand">{tag.name}</span></label>)}</div><input type="hidden" name="tags" value={selectedTags.join(",")} /></fieldset>
