...
export function LongCourseGuide({ guide, geojson, courseType, surface }: { guide: CourseGuide; geojson: LineString; courseType: CourseType; surface: Surface }) {
  ...
  return <div className="space-y-10 pb-20">
    ...
    <nav aria-label="ページ内ナビゲーション" className="sticky top-16 z-40 -mx-4 flex border-y border-line bg-paper/95 px-4 backdrop-blur sm:mx-0 sm:rounded-lg sm:border"><a href="#overview" className="flex-1 py-3 text-center text-sm font-bold">概要</a><a href="#route" className="flex-1 py-3 text-center text-sm font-bold">ルート</a><a href="#highlights" className="flex-1 py-3 text-center text-sm font-bold">{guide.checkpointsTitle ?? "見どころ"}</a><button onClick={() => setSheetOpen(true)} className="flex-1 py-3 text-center text-sm font-bold">アクセス</button></nav>
    ...
    <section id="highlights" className="scroll-mt-32 space-y-4"><div><h2 className="border-l-4 border-brand pl-3 text-xl font-bold sm:text-2xl">{guide.checkpointsTitle ?? "見どころ"}</h2><p className="mt-2 text-sm text-sub">{selectedStart.name}からの順番。カードを開くと詳しく見られます。</p></div><div className="-mx-4 flex snap-x gap-4 overflow-x-auto px-4 pb-3 sm:mx-0 sm:px-0">{checkpoints.map((checkpoint, index) => <details key={checkpoint.id} className="group w-[82%] shrink-0 snap-center overflow-hidden rounded-xl border border-line bg-paper sm:w-[44%]"><summary className="cursor-pointer list-none">{checkpoint.photo ? <div className="relative"><SpotImage src={checkpoint.photo.url} alt={checkpoint.photo.alt} width={640} height={360} sizes="(min-width: 768px) 44vw, 82vw" unoptimized className="aspect-video w-full object-cover" /><span className="absolute left-3 top-3 rounded-full bg-ink px-2.5 py-1 text-xs font-bold text-paper">{index + 1}・{(checkpoint.displayDistanceM / 1000).toFixed(1)}km</span></div> : <div className="flex aspect-video items-end bg-cream p-4"><span className="rounded-full bg-ink px-2.5 py-1 text-xs font-bold text-paper">{index + 1}・{(checkpoint.displayDistanceM / 1000).toFixed(1)}km</span></div>}<div className="p-4"><h3 className="text-lg font-bold">{checkpoint.name}</h3><p className="mt-1 text-xs text-sub">タップして詳細を見る</p></div></summary><div className="border-t border-line p-4"><p className="text-sm leading-6 text-sub">{checkpoint.description}</p><div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-cream px-2.5 py-1 font-bold">{checkpoint.surfaceLabel}</span><span className="rounded-full border border-line px-2.5 py-1">{checkpoint.caution}</span></div>{checkpoint.photo && <a href={checkpoint.photo.sourceUrl} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center gap-1 text-[11px] text-sub underline">写真: {checkpoint.photo.credit} / {checkpoint.photo.license}<ExternalLink size={11} /></a>}</div></details>)}</div></section>

    <div className="fixed inset-x-0 bottom-0 z-50 border-t border-line bg-paper/95 p-3 backdrop-blur"><div className="mx-auto grid w-full max-w-2xl grid-cols-[minmax(0,1fr)_auto] gap-2"><button onClick={() => setSheetOpen(true)} className="flex min-h-14 min-w-0 items-center justify-center gap-2 rounded-xl bg-ink px-4 font-black text-paper shadow-lg"><MapPin size={20} className="shrink-0" /><span className="truncate">{hasStartChoice ? "スタートを決める" : "アクセスを確認"}</span></button><a href={selectedStart.gpxHref} download onClick={() => track("gpx_download", { slug: guide.slug, startId: selectedStart.id, source: "fixed_action" })} className="flex min-h-14 items-center justify-center gap-2 rounded-xl border-2 border-ink bg-paper px-4 font-black text-ink"><Download size={20} className="shrink-0" /><span className="hidden sm:inline">GPXをダウンロード</span><span className="sm:hidden">GPX</span></a></div></div>
