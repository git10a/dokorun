...
export function LongCourseGuide({ guide, geojson, courseType, surface }: { guide: CourseGuide; geojson: LineString; courseType: CourseType; surface: Surface }) {
  ...
  return <div className="space-y-10 pb-20">
    ...
    <div className="fixed inset-x-0 bottom-0 z-50 border-t border-line bg-paper/95 p-3 backdrop-blur"><button onClick={() => setSheetOpen(true)} className="mx-auto flex min-h-14 w-full max-w-lg items-center justify-center gap-2 rounded-xl bg-ink px-5 font-black text-paper shadow-lg"><MapPin size={20} />{hasStartChoice ? "最寄駅からスタートを決める" : "最寄駅からアクセスを確認"}</button></div>
