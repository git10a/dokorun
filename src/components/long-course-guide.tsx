...
export function LongCourseGuide({ guide, geojson, courseType, surface }: { guide: CourseGuide; geojson: LineString; courseType: CourseType; surface: Surface }) {
  ...
  return <div className="space-y-10">
    ...
    <div className="fixed inset-x-0 bottom-0 z-50 border-t border-line bg-paper/95 p-3 backdrop-blur"><div className="mx-auto flex w-full max-w-2xl gap-2">
      <button onClick={() => setSheetOpen(true)} className="flex min-h-14 flex-1 items-center justify-center gap-2 rounded-xl bg-ink px-4 font-black text-paper shadow-lg"><MapPin size={20} className="shrink-0" /><span>{hasStartChoice ? "スタート地点を決める" : "アクセスを確認"}</span></button>
      <Link href={`/spots/${guide.slug}?tab=logs#dokolog`} onClick={() => track("footer_checkin_cta", { slug: guide.slug })} className="flex min-h-14 shrink-0 items-center justify-center rounded-xl bg-brand px-4 font-black shadow-lg">走ったよ 🏃</Link>
    </div></div>
