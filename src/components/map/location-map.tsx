 ...
 */
export function LocationMap({ lat, lng, name, embedUrl }: { lat: number; lng: number; name: string; embedUrl?: string }) {
  ...
  const src = embedUrl ?? `https://www.google.com/maps/embed/v1/place?key=${key}&q=${encodeURIComponent(query)}&zoom=15&language=ja&region=JP`;
  return (
