...
export async function POST(request: Request) {
  ...
  if (on) await db.insert(hashiritai).values({ clientId, spotId: spot[0].id }).onConflictDoNothing();
  else await db.delete(hashiritai).where(and(eq(hashiritai.clientId, clientId), eq(hashiritai.spotId, spot[0].id)));
