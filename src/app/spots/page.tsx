    ...
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-6">
      ...
      <div className="grid gap-6 md:grid-cols-[minmax(0,3fr)_minmax(340px,2fr)]">
        <div className="min-w-0 space-y-6">
          ...
          <div className="flex items-center justify-between"><p className="font-bold"><span className="text-2xl">{result.total}</span>件のスポット</p></div>
          {result.spots.length ? <div className="grid gap-5 sm:grid-cols-2">{result.spots.map((spot) => <SpotCard key={spot.id} spot={spot} />)}</div> : <div className="rounded-xl border border-line bg-cream px-5 py-16 text-center"><p className="font-bold">条件に合うスポットが見つかりませんでした</p><Link href="/spots" className="mt-5 inline-block rounded-lg bg-brand px-5 py-2.5 font-bold">条件をクリア</Link></div>}
