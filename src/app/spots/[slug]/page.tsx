...
export default async function SpotDetailPage({ params, searchParams }: { params: Params; searchParams: Promise<{ logs?: string; posted?: string; run?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-10 px-4 py-8 md:px-6">
      ...
      <section><div className="mb-5 flex items-center justify-between gap-3"><h2 className="border-l-4 border-brand pl-3 text-xl font-bold sm:text-2xl">設備</h2><Link href={`/spots/${spot.slug}/edit`} className="text-sm font-bold text-accent">情報修正</Link></div><FacilityIcons spot={spot} /></section>
      ...
      <section className="rounded-2xl border border-line px-5 py-7 text-center">
        <p className="font-bold">スポット情報に間違いや変わったところを見つけたら</p>
        <p className="mt-2 text-sm text-sub">設備・紹介文・コース情報は、ドコランのユーザーなら誰でも修正できます。</p>
        <Link href={`/spots/${spot.slug}/edit`} className="mt-4 inline-block rounded-lg border border-line bg-paper px-7 py-3 font-bold hover:bg-cream">✏️ 情報修正はこちら</Link>
      </section>
