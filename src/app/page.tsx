...
import { TodayRunChoices } from "@/components/today-run-choices";
import { siteConfig } from "@/lib/site";

// cookies/headers/searchParamsを読まず表示は全ユーザー共通のためISR化。
// 定番/新着スポットや件数の反映が最大1時間遅れるのみ
...
export default async function HomePage() {
  const [{ classic: classicSpots, newest: newSpots }, tags, counts] = await Promise.all([getHomeSpots(), getSearchTags(), getPrefectureCounts()]);
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl space-y-16 px-4 pt-12 md:px-6">
        ...
        <section><div className="mb-6 flex items-end justify-between gap-4"><div className="space-y-1"><h2 className="border-l-4 border-brand pl-3 text-xl font-bold sm:text-2xl">定番スポット</h2><p className="pl-4 text-xs leading-5 text-sub sm:text-sm">迷ったらここから。走り方を組み立てやすい定番の場所です。</p></div><Link href="/spots" className="flex shrink-0 items-center gap-1 text-sm font-bold text-accent">すべて見る <ArrowRight size={16} /></Link></div><div className="grid gap-5 lg:grid-cols-2">{classicSpots.map((spot) => <SpotCard key={spot.id} spot={spot} />)}</div></section>
