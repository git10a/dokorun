  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl space-y-16 px-4 pt-12 md:px-6">
        <section><div className="mb-6 flex items-end justify-between gap-4"><h2 className="border-l-4 border-brand pl-3 text-xl font-bold sm:text-2xl">大会からさがす</h2><Link href="/races" className="flex items-center gap-1 text-sm font-bold text-accent">すべて見る <ArrowRight size={16} /></Link></div><p className="-mt-3 mb-5 text-sm text-sub">マラソン遠征の試走・前日ランに使えるコースを大会別にまとめています。</p><div className="flex flex-wrap gap-2.5">{racesByCalendar.map((race) => <Link key={race.slug} href={`/races/${race.slug}`} className="rounded-full border border-line bg-paper px-4 py-2 text-sm font-bold hover:bg-cream">🏁 {race.name}<span className="ml-1.5 text-xs font-medium text-sub">{race.timing.replace("毎年", "")}</span></Link>)}</div></section>
